...
func (r *memberRepository) List(executor intf.Executor, options ...intf.QueryOption) ([]model.MemberInfo, error) {
	...
	var rows []entity.MemberProfileIncludeRow
	if err := executor.Find(&rows).Error; err != nil {
		return nil, err
	}

	result := make([]model.MemberInfo, len(rows))
	for i, row := range rows {
		result[i] = entity.ToMemberInfo(row)
	}
...
func (r *memberRepository) GetProfile(executor intf.Executor, options ...intf.QueryOption) (model.MemberInfo, error) {
	executor = r.withTransaction(executor)
	executor = executor.Table(entity.MemberTable).Where("member_table.deleted_at IS NULL")
	for _, opt := range options {
		executor = opt(executor)
	...
	var row entity.MemberProfileIncludeRow
	if err := executor.First(&row).Error; err != nil {
		return model.MemberInfo{}, err
	}

	return entity.ToMemberInfo(row), nil
}
...
func (r *memberRepository) Update(executor intf.Executor, update model.MemberUpdate) error {
	...
	updates := map[string]any{
		...
		"assigned_admin_at":        update.AssignedAdminAt,
	}
