...
所有 model 对象都必须使用 New 构造函数进行构造（value 对象除外，其必须使用 literal 构造），禁止直接使用 struct literal 进行构造，否则会导致代码风格不统一，增加代码维护难度。只有 Args 类对象不需要 New 构造函数，因为这类都是直接由 ReadJSON 反序列化构造的，不需要 New 构造函数，否则会导致代码冗余和不必要的复杂性。
