...
type WorksetApp interface {
	...
	List(cx context.Context, currUid string, args *val.ListWorksetArgs) app_res.AppRes[[]val.WorksetVal]

	// `Create` creates a new workset inside a team.
	// `currUid` must be an admin of the team.
	Create(cx context.Context, currUid string, args *val.CreateWorksetArgs) app_res.AppRes[val.WorksetCreatedRes]
