...
func (a *userStatsAppImpl) GetStats(cx context.Context, currUid string) app_res.AppRes[val.UserStats] {
	lgr := app_util.TakeLgr(cx)

	if currUid == "" {
		return app_res.Reject[val.UserStats](app_res.BadRequest, "curr_uid 不能为空")
	}

	stats, err := a.userStatsRepo.EnsureGet(currUid)
	if err != nil {
		if repo_infra.IsTimeout(err) {
			return app_res.Reject[val.UserStats](app_res.Timeout, "获取用户统计超时")
		}

		if repo_infra.IsUnavailable(err) {
			return app_res.Reject[val.UserStats](app_res.Unavailable, "统计服务暂不可用")
		}

		lgr.Error("[userStatsAppImpl.GetStats] failed to get user stats", zap.Error(err))
		return app_res.Reject[val.UserStats](app_res.ServerError, "获取用户统计失败")
	}

	return app_res.Accept(&val.UserStats{
		UserId:                stats.UserId,
