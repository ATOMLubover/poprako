...
## 更新摘要
**变更内容**
- 新增前端可配置端口支持（FRONTEND_PORT、FRONTEND_PREVIEW_PORT、FRONTEND_HOST）
- 新增动态API基础URL解析（VITE_API_BASE_URL）
- 更新前端环境变量配置说明
- 增强前端开发服务器灵活性
...
ENV["frontend/src/env.d.ts<br/>环境变量类型声明"]
HTTP["frontend/src/api/http.ts<br/>API 客户端"]
end
subgraph "工具"
JF["justfile<br/>任务编排"]
...
- [frontend/src/api/http.ts:20-27](file://frontend/src/api/http.ts#L20-L27)
...
- 前端构建与开发
  - Vite 提供开发服务器与热重载，支持可配置端口和主机绑定；Vue 插件自动注册，路径别名 @ 指向 src。
- 动态API配置
  - 前端支持通过 VITE_API_BASE_URL 环境变量动态配置 API 基础URL，默认为 "/api/v1"。
- 工具链与任务
...
- [internal/infrastructure/repository/repository.go:11-29](file://internal/infrastructure/repository/repository.go#L11-L29)
- [frontend/vite.config.ts:12-19](file://frontend/vite.config.ts#L12-L19)
- [frontend/src/env.d.ts:6-8](file://frontend/src/env.d.ts#L6-L8)
- [frontend/src/api/http.ts:20-27](file://frontend/src/api/http.ts#L20-L27)
- [justfile:6-38](file://justfile#L6-L38)

## 架构总览
下图展示了开发环境中的关键交互：前端通过可配置的 Vite 开发服务器访问后端 API；后端从 .env 与 app_config.json 读取配置，连接 PostgreSQL 数据库；前端支持动态API基础URL配置，增强开发灵活性。

```mermaid
sequenceDiagram
participant Dev as "开发者"
participant FE as "前端开发服务器(Vite)<br/>可配置端口"
participant BE as "后端(HTTP 服务器)"
participant DB as "PostgreSQL"
Dev->>FE : 访问 http : //host : port<br/>可配置端口
FE->>BE : 请求 VITE_API_BASE_URL + /api/v1/*
...
- [internal/infrastructure/repository/repository.go:11-29](file://internal/infrastructure/repository/repository.go#L11-L29)
- [frontend/vite.config.ts:21-42](file://frontend/vite.config.ts#L21-L42)
- [frontend/src/api/http.ts:20-27](file://frontend/src/api/http.ts#L20-L27)
...
- 开发服务器与热重载
  ...
  - 支持可配置端口：FRONTEND_PORT（开发端口，默认5173）、FRONTEND_PREVIEW_PORT（预览端口，默认4173）。
  - 支持可配置主机：FRONTEND_HOST（默认127.0.0.1）。
- 动态API基础URL
  - 通过 VITE_API_BASE_URL 环境变量配置API基础URL，默认为 "/api/v1"。
  - 支持相对路径和绝对路径配置。
- 路径别名与类型声明
  - 配置 @ 到 src；类型声明文件提供 import.meta.env.VITE_API_BASE_URL 类型提示。

**更新** 新增可配置端口和动态API基础URL支持

**章节来源**
- [frontend/package.json:1-36](file://frontend/package.json#L1-L36)
- [frontend/vite.config.ts:8-42](file://frontend/vite.config.ts#L8-L42)
- [frontend/src/env.d.ts:6-8](file://frontend/src/env.d.ts#L6-L8)
- [frontend/src/api/http.ts:20-27](file://frontend/src/api/http.ts#L20-L27)
...
- 前端
  - Vite 默认开发服务器端口通常为 5173；如需自定义端口，设置 FRONTEND_PORT 环境变量。
  - 预览服务器端口默认为 4173；如需自定义，设置 FRONTEND_PREVIEW_PORT 环境变量。
  - 主机绑定默认为 127.0.0.1；如需外部访问，设置 FRONTEND_HOST 环境变量。
  - API基础URL默认为 "/api/v1"；如需自定义后端地址，设置 VITE_API_BASE_URL 环境变量。
- API 文档
...
- Vue
  ...
  - 环境变量：在前端项目中配置 VITE_API_BASE_URL、FRONTEND_PORT、FRONTEND_PREVIEW_PORT、FRONTEND_HOST。
- 代理
  - 前端无需代理配置，因为支持动态API基础URL解析。
...
- 前端端口冲突
  - 设置 FRONTEND_PORT 环境变量选择其他端口；默认5173。
- 前端预览端口冲突
  - 设置 FRONTEND_PREVIEW_PORT 环境变量选择其他端口；默认4173。
- 前端API地址错误
  - 设置 VITE_API_BASE_URL 环境变量配置正确的API基础URL。
- 前端主机绑定问题
  - 设置 FRONTEND_HOST 环境变量配置主机地址，默认127.0.0.1。
...
- [justfile:24-35](file://justfile#L24-L35)
- [script/seed/README.md:3-15](file://script/seed/README.md#L3-L15)
- [frontend/vite.config.ts:8-25](file://frontend/vite.config.ts#L8-L25)
- [frontend/src/api/http.ts:20-27](file://frontend/src/api/http.ts#L20-L27)

## 结论
通过本指南，您可以在本地快速搭建 poprako-web-ms 的开发环境：准备 Go 与 Node.js 环境，配置数据库与环境变量，安装依赖，启动前后端服务，并使用 Justfile 与种子脚本完成数据库初始化。新增的可配置端口和动态API基础URL功能进一步提升了开发体验，避免端口冲突并增强了部署灵活性。遇到问题时，可依据故障排除指南逐项排查。
...
- JWT_SECRET_KEY：JWT 密钥
- DATABASE_URL：PostgreSQL 连接字符串
- FRONTEND_PORT：前端开发服务器端口，默认5173
- FRONTEND_PREVIEW_PORT：前端预览服务器端口，默认4173
- FRONTEND_HOST：前端服务器主机地址，默认127.0.0.1
...
- frontend/vite.config.ts：前端开发服务器与构建配置，支持可配置端口
- frontend/src/env.d.ts：前端环境变量类型声明，包含VITE_API_BASE_URL
- frontend/src/api/http.ts：API客户端配置，支持动态API基础URL解析
...
### 前端API调用示例
前端API调用通过 httpClient 实例进行，支持动态基础URL配置：

```typescript
// 默认使用 "/api/v1" 作为基础URL
const response = await httpClient.post<UserData>('/users/login', userData);

// 支持自定义基础URL
// 设置 VITE_API_BASE_URL=http://localhost:8080
// 将使用 http://localhost:8080/api/v1/users/login
```

**章节来源**
- [frontend/src/api/http.ts:99-135](file://frontend/src/api/http.ts#L99-L135)
