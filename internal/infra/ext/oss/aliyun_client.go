...
func (a *aliyunClient) GeneratePutPresignedURL(objectKey string) (string, error) {
	...
	result, err := a.client.Presign(context.TODO(), input, oss.PresignExpires(exp))
	if err != nil {
...
func (a *aliyunClient) GenerateGetPresignedURL(objectKey string) (string, error) {
	if a.customDomain != "" {
		return fmt.Sprintf("https://%s/%s", a.customDomain, objectKey), nil
	}

	return "", fmt.Errorf("未配置自定义域名")
}

func (a *aliyunClient) Delete(objectKey string) error {
	const maxRetries = 3
...
func (a *aliyunClient) DeleteBatch(objectKeys []string) error {
	if len(objectKeys) == 0 {
		return nil
	}
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		_, err := a.client.DeleteMultipleObjects(ctx, &oss.DeleteMultipleObjectsRequest{
			Bucket: oss.Ptr(a.bucketName),
			Delete: &oss.Delete{
				Objects: objects,
				Quiet:   true,
