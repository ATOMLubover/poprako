...
//
// @Success 	200 {object} []value.WorksetInfo
//
// @Router 		/worksets [get]
func ListWorksets(appState *state.AppState) iris.Handler {
	worksetApplication := appState.WorksetApplication
...
// @Accept 	json
// @Produce 	json
// @Param 		body body value.CreateWorksetArgs true "创建工作集参数"
//
// @Success 	201 {object} value.CreateWorksetResult
//
...
// @Param 		body body value.UpdateWorksetArgs true "更新工作集参数"
//
// @Success 	200
//
...
func UpdateWorkset(appState *state.AppState) iris.Handler {
	...
	return func(ctx iris.Context) {
		...
		var args value.UpdateWorksetArgs
		if err := ctx.ReadJSON(&args); err != nil {
			reject(ctx, iris.StatusBadRequest, "请求体格式错误: "+err.Error())
			return
		}
		...
		if err := worksetApplication.UpdateWorkset(
			buildTraceScope(ctx),
			currentUserID,
			args,
		); err != nil {
...
func DeleteWorkset(appState *state.AppState) iris.Handler {
	...
	return func(ctx iris.Context) {
		...
		if err := worksetApplication.DeleteWorkset(
			buildTraceScope(ctx),
			currentUserID,
			worksetID,
		); err != nil {
